...
public class HttpClientUtil {
	...
	public static void downloadGet(String url,Map<String,String> data,String filePath) throws Exception{
		...
		try{
			...
			URIBuilder uriBuilder = new URIBuilder().setScheme("http").setHost(url == null ? null : url.trim());
	...
	public static String sendPost(String url,Map<String, Object> data) throws Exception{
		CloseableHttpClient httpClient = HttpClients.createDefault();
		try{
			HttpPost post = new HttpPost(url == null ? null : url.trim());
			if(!CollectionUtils.isEmpty(data)){
	 ...
	 * @throws Exception
	 */
	public static String sendPostString(String url,Map<String, String> data) throws Exception{
		CloseableHttpClient httpClient = HttpClients.createDefault();
		try{
			HttpPost post = new HttpPost(url == null ? null : url.trim());
			if(!CollectionUtils.isEmpty(data)){
    ...
    public static String sendPost(String url,Map<String, Object> data,String charset) throws Exception{
        ...
        try{
            ...
            if(!CollectionUtils.isEmpty(data)){
                ...
                HttpEntity fromEntity = new UrlEncodedFormEntity(params, (!StringUtils.isEmpty(charset))?charset:CONTENT_CHARSET);
                post.setEntity(fromEntity);
            }
            HttpResponse response = httpClient.execute(post);
            if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
	...
	public static String sendHttpsGet(String url,Map<String,String> data) throws Exception{
		...
		try{
			...
			URIBuilder uriBuilder = new URIBuilder().setScheme("https").setHost(url == null ? null : url.trim());
			if(!CollectionUtils.isEmpty(data)){
				for (String key : data.keySet()) {
					uriBuilder.setParameter(key, data.get(key));
				}
...
//		try {
//			String result = sendStringHttpsPost("http://localhost:8101/channel/aceept.html?ss=123&ddd=2222","测试字符串\n你大爷你大爷");
